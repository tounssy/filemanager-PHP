...
var dossierEx = document.getElementById("fileContent"); //Ici on recupère la div de notre HTML
        ...
        if (event.target.tagName == "A") { //Si la cible de notre click est une balise A 
            if (event.target.innerText == "..") { // Et SI notre event contient ".."
                if (dirRoot != "./") { //Et ENCORE SI notre variable (ligne 2)(chemin de navigation) est différente de la base racine
                    ...
                    console.log(dirRoot);    
                        ...
                        dirRoot = dirRoot + event.target.innerText + '/'; //Rebelotte (On a rajouté le caractère "/" pour que la navigation marche) (?)
                }
